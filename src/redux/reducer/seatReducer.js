...
const seatReducer = (state = defaultState, action) => {
    switch (action.type) {
        case 'CHOOSE_SEAT': {
            const selectedSeat = [...state.seatSelectedList];
            const findItem = selectedSeat.findIndex(item => item.soGhe === action.payload.soGhe);

            if (findItem !== -1) {
                selectedSeat.splice(findItem, 1);
            } else {
                selectedSeat.push(action.payload);
            };

            return { ...state, seatSelectedList: selectedSeat }
        }
        case 'PAY': {
            const data = state.seatList.map(item => {
                const listSeat = item.danhSachGhe.map(seat => {
                    state.seatSelectedList.forEach(ele => {
            ...
            return {
                ...state, seatList: data, seatSelectedList: []
            }
        }
        default:
            return state;
